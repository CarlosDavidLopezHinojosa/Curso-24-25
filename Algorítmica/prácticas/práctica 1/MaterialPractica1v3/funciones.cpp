                                        ...
                                        const std::vector<double>& tiemposEstimados)
{
    ...
    // return 1 - SSR/SST;
    size_t n = tiemposEstimados.size();
    double mediaReal = std::accumulate(tiemposReales.begin(),tiemposReales.end(),0);
    double mediaEstimada = std::accumulate(tiemposEstimados.begin(),tiemposEstimados.end(),0);

    std::vector<double> sumatorioCuadradoReal = tiemposReales;
    std::vector<double> sumatorioCuadradoEstimado = tiemposEstimados;

    for (size_t i = 0; i < n; i++)
    {
        sumatorioCuadradoReal[i] = (sumatorioCuadradoReal[i] - mediaReal) * (sumatorioCuadradoReal[i] - mediaReal) / n;
        sumatorioCuadradoEstimado[i] = (sumatorioCuadradoEstimado[i] - mediaEstimada) * (sumatorioCuadradoEstimado[i] - mediaEstimada) / n;
    }
    

    double varianzaReal = std::accumulate(sumatorioCuadradoReal.begin(),sumatorioCuadradoReal.end(),0);
    double varianzaEstimada = std::accumulate(sumatorioCuadradoEstimado.begin(),sumatorioCuadradoEstimado.end(),0);
                  ...
                  std::vector<double>& tiemposEstimados)
{
    ...
    for (size_t i = 0; i < tiemposReales.size(); i++)
        fichero << numeroElementos[i] << " " << tiemposReales[i] << " " 
                << numeroElementos[i] << " " << tiemposEstimados[i] << "\n";
    fichero.close();
    assert(!fichero.is_open());
}
                              ...
                              std::vector<double>& numeroElementos)
{
    ...
    Clock reloj;
    size_t curr = 0;
    for (size_t i = nMin; i < n; i += incremento)
    {   
...
void tiempoMultiplicacionMatrices(int nMin, int nMax, int repeticiones, 
                                  ...
                                  std::vector<double>& numeroElementos)
{
    tiemposReales.resize(nMax + 1);
    numeroElementos.resize(nMax + 1);
